...
class Planning:
    ...
    def plan_next_action(self, current_pos: Tuple[int, int], current_dir: str, world) -> str:
        ...
        2. If holding gold, plan path back to (0,0) — fallback to risky path if no safe route.
        ...
        if world.has_gold:
            if not self.current_plan:
                # First try a strictly safe path
                path_home = self.find_path(current_pos, (0, 0), world, strict_safe=True)
                # If no safe path exists, try a risky one
                if not path_home:
                    path_home = self.find_risky_path(current_pos, (0, 0), world)
                if path_home:
        ...
        safe_unvisited = [
            cell for cell in self.logic_inference.safe_cells
            if cell not in self.logic_inference.visited_cells
        ]
        if safe_unvisited:
        ...
        if world.has_arrow and self.logic_inference.wumpus_cells:
            ...
            # Move into shooting position if safe
        ...
        # 5. Risky move to warning cell if no safe cells left
    ...
    def find_path(self, start: Tuple[int, int], goal: Tuple[int, int], world, strict_safe: bool = False) -> List[Tuple[int, int]] | None:
        """
    ...
    def find_risky_path(self, start: Tuple[int, int], goal: Tuple[int, int], world) -> List[Tuple[int, int]] | None:
        """
    ...
    def _can_shoot_wumpus(self, agent_pos: Tuple[int, int], agent_dir: str, wumpus_pos: Tuple[int, int]) -> bool:
        """
