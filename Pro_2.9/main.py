...
# Default config (can be overwritten by user input)
world_size = 8
k = 2
pit_prob = 0.2
current_agent_type = "Hybrid"
...
def main():
    ...
    auto_move_timer = 0
    auto_move_delay = 190  # milliseconds
    ...
    print("Controls: SPACE (step), TAB (toggle mode), R (reset)")
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if action == "reset_game":
                    world, agent, ui = _reset_game(world.seed)
                    agent_action_count = 0
                    moving_module = MovingWumpusModule(world)
                    game_over = False
                    step_trigger = False
                    ...
                    world, agent, ui = _reset_game(new_seed)
                    agent_action_count = 0
                    moving_module = MovingWumpusModule(world)
                    game_over = False
                    step_trigger = False
                    ...
                    if ui.agent_type == "Hybrid":
                        ...
                        agent = RandomAgent(world.grid_size)
                    current_agent_type = ui.agent_type
                ...
                if event.key == pygame.K_TAB:
                    if ui.mode == GameMode.STEP:
                        ui.mode = GameMode.AUTO
                        print("🔁 Switched to AUTO mode")
                    else:
                        ui.mode = GameMode.STEP
                        print("🧽 Switched to STEP mode")
                elif event.key == pygame.K_SPACE:
                    if ui.mode == GameMode.STEP:
                        step_trigger = True

        # STEP mode execution
        if ui.mode == GameMode.STEP and step_trigger and world.is_game_over() == "continue":
            agent.update_knowledge(world.agent_pos, world.percepts, world)
            action = agent.plan_next_action(world.agent_pos, world.agent_dir, world)
            if action == "wait":
                print("🔃 Agent is waiting: no safe moves found.")
            else:
...
def get_user_config(screen):
    ...
    input_boxes = [
        {"label": "Map Size (4-10):", "value": "", "rect": pygame.Rect(250, 100, 100, 32), "type": int, "min": 4, "max": 10},
